...
async function main() {
  ...
  const manifestPath = resolve(parsed.options.workflowJson ?? join(scriptDir(), "workflow.json"));
  ...
  const context = {
    ...
    runsRoot: join(homedir(), ".getsuperpower", "runs", manifest.name),
    json: parsed.options.json === true,
  };

  switch (parsed.command) {
    case "start":
      await startRun(context, parsed.options);
      return;
    case "status":
      await showStatus(context, parsed.options);
      return;
    case "log":
      await logEventCommand(context, parsed.options);
      return;
    case "advance":
      await advanceRun(context, parsed.options);
      return;
    case "summary":
      await writeSummaryCommand(context, parsed.options);
      return;
    default:
      throw new Error(`Unknown loop command: ${parsed.command}`);
  }
}
...
function writeOutput(context, payload) {
  ...
  console.log(`${payload.workflow} ${payload.runId}`);
  ...
  if (payload.summaryPath) {
    console.log(`Summary: ${payload.summaryPath}`);
  }
  console.log("Actions:");
